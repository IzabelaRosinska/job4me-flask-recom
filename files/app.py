...
from recommendation_db import Recommender
...
# labels_data = read_json("files/labels_IT_old.json")
...
# offers_data = read_jsonl("files/offers_all.jsonl")
offers_data = read_json("files/offers_final.json")
...
def tesst():
    response = ""
    if a := request.args.get('a'):
        response += 'a=' + a
    if b := request.args.get('b'):
        response += 'b=' + b
    return response
...
#     ranking = recommender.get_offers_ranking(employee, {}, branches_weights)[:5]
...
def process(offer_id):
    ...
    #     return "Success"
    #     # offer = offers_data[offer_id]
    #     # recommender.load_and_save_offer(offer, get_by_id('files/offers.json', offer_id), branches_weights)
    #     # return "Success"
